...
def make_dir(type):
    ...
    for json_annotation in listdir:
        ...
        elif 'P02.리어범퍼' in temp_list or 'P11.리어램프' in temp_list: #해당 이미지 내에 리어범퍼/램프가 있는경우
            for part in car_parts: 
                if part['classId'] == 'P02.리어범퍼' or part['classId'] =='P11.리어램프':
                    # 각 parts의 좌표값을 가져온다.
                    xmin = int(part['coords']['tl']['x'])
                    ymin = int(part['coords']['tl']['y'])
                    xmax = int(part['coords']['br']['x'])
                    ...
                    if (xmax - xmin) > 4*(ymax - ymin): #리어램프나 범퍼의 크기가 조건에 맞는 경우
                        if 'P00.차량전체' in temp_list:
                            path_arr = np.fromfile(img_dir, np.uint8)
                            img = cv2.imdecode(path_arr, cv2.IMREAD_COLOR)
                            for part in car_parts:
                                if part['classId'] == 'P00.차량전체':
...
if __name__ == "__main__":
    ...
    BASE_DIR = r"C:\Users\mok\Project2\eff_rear_kia"     
